...
namespace EXPRT
{
    class DB_UTILS
    {
        ...
        public static MySqlDataReader ReturnReportsEmp(string ssn)
        {
            string connect = "server=localhost;database=expense_report_sys;uid=root;pwd=*******;";
            ...
            using (var conn = new MySqlConnection(connect))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    string x = String.Format("SELECT manager_id, expense_type, expense_reason, date_filed, price FROM employees WHERE ssn={0}", ComputeSha256Hash(ssn));
                    cmd.CommandText = x;
        ...
        public static MySqlDataReader ReturnReportsMan(int manager_id)
        {
            string connect = "server=localhost;database=expense_report_sys;uid=root;pwd=*******;";
            ...
            using (var conn = new MySqlConnection(connect))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    string x = String.Format("SELECT ssn, expense_type, expense_reason, date_filed, price FROM employees WHERE manager_id={0}", manager_id);
                    cmd.CommandText = x;
